...
export const consultaController = async (req, res) => {
  ...
  try {
    ...
    validarCampos(req, res) // Validaciones
    
    const hardLimit = Math.max(1, Math.min(Number(limit), 100)) // Tope defensivo
    
    let sql = await consultarGemini({ question, schemaDDL: demoDDL, limit: hardLimit })
    
    /* Desarmo el markdown que devuelve gemini */
    cleanSql = normalizeGeneratedSql(sql)

    if (!cleanSql) {
      return res.status(400).json({ error: 'Hubo un error en la consulta', sql: sql, detail: 'La consulta SQL generada por Gemini es inválida, porfavor inténtelo de nuevo reformulando el ingreso de datos' })
    }
